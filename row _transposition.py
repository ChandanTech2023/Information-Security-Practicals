def cipher_encryption():
    ...
    for k in range(len(key)):  # Loop through each column in the sorted order
        d = int(num_loc[k])    # Get the actual index of the column to read
        for j in range(num_of_rows): # Loop down through every row of that column
            cipher_text += arr[j][d]
